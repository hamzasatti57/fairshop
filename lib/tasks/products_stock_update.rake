  ...
  task update_stock: :environment do
    ...
    data["DocumentElement"]["StockOnHand"].each do |stock|
      ...
      if @product.present?
        ...
        ProductColor.create!(product_id: @product.id, color_id: @color.id, stock_item_id: stock["StockItemID"]) unless ProductColor.where(product_id: @product.id, color_id: @color.id).present? || @color.blank?
